...
#---functions---------------------
def command():
	Parser = argparse.ArgumentParser(prog='AlternateReferenceMaker.py', usage='%(prog)s FASTA VCF', 
																	 description='%(prog)s is used to alternate a FASTA sequence with variants called in a VCF file',
																	 formatter_class=argparse.RawTextHelpFormatter)
	Parser.add_argument('INPUT', nargs=2, help='name of input FASTA file and name of VCF file, \nplease add directory if file is not in current working directory')
...
# converting sequence into one string, adding '#' so sequence starts with 1 and not 0
	...
	for entry in VCFinfo:
		...
		if lala == Ori:
			...
			if Alt.find('I')!=-1: #fckng PacBio VCF output
				Alt = Alt.strip('I')
